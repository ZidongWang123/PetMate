...
const App = () => {
  ...
  const getGroupsArticlesRequest=async ()=>{
    ...
    const articles=articlesResulet.length?articlesResulet.map(item=>{
      return { "id":item._id,"Topics":item.title,"Date":FormData(item.date) ,   "Author":item.u_id.name}
    }):[]
  ...
  return (
    ...
    <div >
      ...
      {group.length!=0&&<GroupDetails group={group} id={groupId}/>}
      {/* {articles.length!=0&&<Forum articles={articles}/>} */}
      {/* {articles.length!=0&&<BasicTable data={articles} columns={columns}></BasicTable>} */}
      {articles.length!=0&&  <TableFilter columns={columns} data={articles} ></TableFilter>}
    </div>
  );
}
